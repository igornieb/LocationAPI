...
def get_places_in_radius(radius: float, lon: float, lat: float, sort_by: str = 'avg_score',page: int = 1, db: Session = Depends(get_db)):
    offset = (page - 1) * paginate_by
    center = func.ST_Point(lon, lat)
    if sort_by == 'avg_score_asc':
        places = db.query(Place).filter(Place.published == True, func.ST_DWithin(Place.location, center, radius)).order_by(asc(Place.avg_score)).offset(offset).limit(paginate_by).all()
    if sort_by == 'avg_score_desc':
        places = db.query(Place).filter(Place.published == True, func.ST_DWithin(Place.location, center, radius)).order_by(desc(Place.avg_score)).offset(offset).limit(paginate_by).all()
    if sort_by == 'distance_asc':
        places = db.query(Place).filter(Place.published == True, func.ST_DWithin(Place.location, center, radius)).order_by(asc(func.ST_Distance(Place.location, center))).offset(offset).limit(paginate_by).all()
    if sort_by == 'distance_desc':
        places = db.query(Place).filter(Place.published == True, func.ST_DWithin(Place.location, center, radius)).order_by(desc(func.ST_Distance(Place.location, center))).offset(offset).limit(paginate_by).all()
    else:
        places = db.query(Place).filter(Place.published == True, func.ST_DWithin(Place.location, center, radius)).offset(offset).limit(paginate_by).all()
    for place in places:
...
def get_places_in_radius_category(category: str, radius: float, lon: float, lat: float, page: int = 1,
    ...
    places = db.query(Place).filter(Place.published == True, Place.category == category,
                                    func.ST_DWithin(Place.location, center, radius)).offset(
...
async def create_places(place: schemas.PlaceCreate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def patch_place(uuid, data: schemas.PlaceEdit, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    place = db.query(Place).filter(Place.id == uuid).first()
    if place is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
